...
export class FileWatcherGateway {
  ...
  async watch(paths: string[]): Promise<FileWatchEventInitialResponse> {
  ...
  private getSubscription(
    subject: Subject<FileWatchEvent>,
    resolve: (value: FileWatchEventInitialResponse) => void,
    ...
    const subscription = subject.pipe(timeout(10_000)).subscribe({
      ...
      complete: () => {
        ...
        const initialResponse: FileWatchEventInitialResponse = { paths: Array.from(initialEvents).map(event => event.path).sort() }
        for (const client of this.clients) {
